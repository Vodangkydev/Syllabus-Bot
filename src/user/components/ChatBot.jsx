...
function ChatBot() {
  ...
  const handleSendMessage = async (message) => {
    ...
    try {
      ...
      // Sử dụng endpoint /chatbot/ask_stream
      const response = await fetch(`http://localhost:8000/chatbot/ask_stream?question=${encodeURIComponent(message)}`, {
        method: "GET",
        headers: {
  ...
  const saveEditMessage = async (index) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/chatbot/ask_stream?question=${encodeURIComponent(editingMessageContent)}&email=${encodeURIComponent(user?.email || "")}`, {
        method: "GET",
        headers: {
          "Content-Type": "text/event-stream",
          "Cache-Control": "no-cache",
      ...
      setState(prev => ({
        ...
        messages: [...newMessages, {
          role: "assistant",
          content: `Xin lỗi, có lỗi xảy ra: ${error.message}. Vui lòng thử lại sau hoặc liên hệ admin để được hỗ trợ.`,
          error: true
        }],
        isLoading: false
      }));
